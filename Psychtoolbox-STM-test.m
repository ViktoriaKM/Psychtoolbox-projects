...
% A simple short-term memory test that first displays 4 images of cats and
% dogs, where the task is to memorize them and select the correct images
% from a set of 9 different pictures. At the end, an accuracy score is
...
% when the counter hits 9, the experiment should stop

% we also need to keep track of which images have been clicked in order to
...
while inClick < 10 
    ...
    for k=1:9 % loops through myRects
        ...
        if withinMyRects && ...
            ...
            % check if the clicked image is within the list of recorded
            % images
            disp(imageClicked);
            containsImage = any(recordedImages(:) == imageClicked);
            if containsImage == 1
...
accuracy = ['Your accuracy is ', num2str((correctImages/inClick)*100), '%'];
disp(accuracy);
